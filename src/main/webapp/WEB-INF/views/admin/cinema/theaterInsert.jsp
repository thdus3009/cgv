		...
		<div id="layoutSidenav_content">
			...
			<main>
				<div class="container-fluid">
					...
					<p style="margin-bottom: 0;">상영관 추가</p>
					<form id="frm" action="./${board}${path}" method="post"
						...
						<div class="form-group">
							...
							<label class="theaterLebel" for="cinemaNum">영화관 선택:</label> <select
								class="cinemaNum selectcinenum" name="cinemaNum">
								<!-- <option value="2d">2D</option>
				   	 	<option value="3d">3D</option> -->
								<c:forEach items="${cine}" var="cnt">
									<option value="${cnt.num}">${cnt.num}/${cnt.name}</option>
								</c:forEach>
							</select> <br> <label class="theaterLebel lebelname" for="name">상영관
								이름:</label> <input type="text" class="form-control" id="name"
								name="name" value="${vo.name}" required="required"> <br> <label
								class="theaterLebel" for="filmType">필름 타입:</label>
					...
					<c:if test="${path eq 'Update'}">
						<button type="button" id="btn_delete" value="${vo.num}">삭제</button>
					</c:if>
